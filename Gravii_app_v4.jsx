...
function MySpaceContent({ dark, connected, onConnect, onNavigate }) {
  ...
  const campaignStatusLabel = (c) => {
    if (c.eligible === true) return "ELIGIBLE";
    if (c.eligible === "ineligible") return "INELIGIBLE";
    if (c.eligible === null) return "UPCOMING";
    if (c.partnerStatus === "INVITE ONLY") return "INVITE ONLY";
    return "REACH TO UNLOCK";
  };

  const CampaignCard = ({ c, idx, opacity = 1 }) => {
    ...
    return (
      <div
        onClick={(e) => { e.stopPropagation(); setExpandedCard(isOpen ? null : key); }}
        style={{
          padding: "24px", border: `1px solid ${isOpen ? "rgba(230,220,250,0.3)" : cardBorder}`, borderRadius: "12px",
          background: isOpen ? "rgba(230,220,250,0.04)" : cardBg, opacity,
          cursor: "pointer", transition: "all 0.2s ease",
        }}
        onMouseEnter={(e) => { if (!isOpen) e.currentTarget.style.background = "rgba(255,255,255,0.06)"; }}
        onMouseLeave={(e) => { if (!isOpen) e.currentTarget.style.background = isOpen ? "rgba(230,220,250,0.04)" : cardBg; }}
      >
        {/* Row 1: 캠페인명 + 상태뱃지 */}
        ...
        {isOpen && (
          <div style={{ marginTop: "18px", paddingTop: "18px", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            {/* CTA */}
            {c.eligible === true && (
              <span onClick={(e) => { e.stopPropagation(); setOptedIn(prev => ({ ...prev, [key]: true })); }} style={{
                fontFamily: outfit, fontWeight: 700, fontSize: "13px", letterSpacing: "2px",
                padding: "12px 28px", borderRadius: "8px",
                cursor: isOpted ? "default" : "pointer", display: "inline-block",
                ...(isOpted
            ...
            {c.eligible === false && c.partnerStatus === "INVITE ONLY" && (
              <span onClick={(e) => e.stopPropagation()} style={{
                fontFamily: outfit, fontWeight: 700, fontSize: "13px", color: "rgba(200,180,240,0.7)", letterSpacing: "2px",
                padding: "12px 28px", border: "1px solid rgba(200,180,240,0.2)", borderRadius: "8px",
                display: "inline-block", cursor: "default",
              }}>INVITE ONLY — REQUEST ACCESS</span>
            )}
            {c.eligible === false && c.partnerStatus !== "INVITE ONLY" && (
              <span onClick={(e) => { e.stopPropagation(); if (onNavigate) onNavigate("discovery"); }} style={{
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                {filteredReach.map((c, i) => (
                  <div key={c.name + c.partner} style={{ animation: `fadeInUp 0.5s ease ${0.85 + i * 0.08}s both` }}>
                    <CampaignCard c={c} idx={i} opacity={0.75} />
                  </div>
                ))}
              </div>
            ))}
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                    {filteredInvite.map((c, i) => (
                      <div key={c.name + c.partner} style={{ animation: `fadeInUp 0.5s ease ${1.0 + i * 0.08}s both` }}>
                        <CampaignCard c={c} idx={i} opacity={0.6} />
                      </div>
                    ))}
                  </div>
                ))}
...
──────────────────────────────────────────── */
const BENEFIT_CATEGORIES = ["All", "General", "Wealth & Finance", "Lifestyle & Retail", "Exclusive Privileges", "Hidden Gems"];
...
const PARTNERS_DATA = [
  ...
  },
  {
    ...
    campaigns: [
      { name: "Community Airdrop", type: "Airdrop", chains: ["All"], category: "General", period: "Mar 1 – Mar 31, 2026", eligible: true, desc: "A surprise token distribution for all active Gravii members. No strings attached — just show up.",
        tags: [{ tier: "All Tiers", type: "open" }] },
    ],
  },
  {
